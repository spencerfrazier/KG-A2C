...
class BERT():
    ...
    def predictNSP(self, s_input=[], s_labels=[], batch_size=8, template_count=80):
        with torch.no_grad():
            ...
            enc_ids = np.array(list(map(self.encodeAndPadNormClassifier,s_input)))
            enc_labels = np.array(list(map(self.encodeAndPadNormClassifier,s_labels_noOBJ)))
    
            # if(len(enc_ids.shape)<2):
            enc_ids_copy = np.zeros([len(enc_ids),self.max_seq_len], dtype ='int64')
            enc_labels_copy = np.zeros([len(enc_ids),self.max_seq_len], dtype ='int64')
            # print(enc_ids.dtype)
            for i in range(len(enc_ids)):
                enc_ids_copy[i] = np.array(enc_ids[i][:self.max_seq_len])
                enc_labels_copy[i] = np.array(enc_labels[i][:self.max_seq_len])
            # print(enc_ids_copy.dtype)
            enc_ids = enc_ids_copy
            # print(sum(enc_ids_copy - enc_ids))
            enc_labels = enc_labels_copy
            # print(enc_ids.shape)
            # if(enc_ids.shape[1] > self.max_seq_len):
            #     enc_ids = enc_ids[:][:self.max_seq_len]
            #     # print("Clipped")
            # if(enc_labels.shape[1] > self.max_seq_len):
            #     enc_labels = enc_labels[:][:self.max_seq_len]
                    # print("Clipped")
                # print(enc_ids)
                # print(enc_ids[0])
                # print(np.array(enc_ids[0]).dtype)
                # print(np.array(enc_ids[0]).shape)
                # print(enc_ids.dtype)
                # enc_ids_copy = np.zeros([1280,128])
                # print(enc_ids.shape)
                # print(enc_ids)
                # for i in range(len(enc_ids)):
                #     enc_ids_copy[i] = np.array(enc_ids[i][:128])
                #     # enc_labels[i] = 
                # print("here")
                # print(enc_ids_copy)
                # print(enc_ids_copy.shape)
                # if(enc_ids_copy.shape[1] > self.max_seq_len):
                #     enc_ids = enc_ids[:][:self.max_seq_len]
                #     # print("Clipped")
                # if(enc_labels.shape[1] > self.max_seq_len):
                #     enc_labels = enc_labels[:][:self.max_seq_len]
