...
def generate_config_files(type_, key, name="base", eval_mode=False):
    with open("comet/config/default.json".format(type_), "r") as f:
        base_config = json.load(f)
    with open("comet/config/{}/default.json".format(type_), "r") as f:
        base_config_2 = json.load(f)
    if eval_mode:
        with open("comet/config/{}/eval_changes.json".format(type_), "r") as f:
            changes_by_machine = json.load(f)
    else:
        with open("comet/config/{}/changes.json".format(type_), "r") as f:
            changes_by_machine = json.load(f)
